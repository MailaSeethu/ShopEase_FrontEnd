...
export class CartComponent implements OnInit, AfterViewInit {
  ...
  //     this.cartservice.Delete(id,deletedcart[0].quantity).subscribe((res) => {
  ...
  DeleteFromCart(id: number) {
    ...
    console.log(id);
    let user = localStorage.getItem('userName') as string;
    let deletedcart = this.cart.filter(cart => cart.productId == id && cart.userName == user);

    if (deletedcart.length > 0) {
      console.log("cart while deleting", deletedcart)
      deletedcart[0].productId = id;
      deletedcart[0].userName = user;
      ...
      this.cartservice.Delete(id, deletedcart[0].quantity).subscribe((res) => {
        ...
        // Update the UserBasedCart and QuentityCount arrays
        const index = this.UserBasedCart.findIndex(product => product.productId === id);
        if (index !== -1) {
          this.UserBasedCart.splice(index, 1); // Remove the product from UserBasedCart
          this.QuentityCount.splice(index, 1); // Remove the quantity from QuentityCount
